...
//      링크 안걸린 제목 처리
		if (songTitleElements.size() != songNumberElements.size()) {
			...
			songTitles.addAll(songTitleElements.stream().filter(v -> !v.getAttribute("title").equals(""))
					.map(v -> v.getText()).collect(Collectors.toList()));
		}
...
//      아티스트 처리
		for (WebElement webElement : form.findElements(By.cssSelector("div#artistName"))) {
			...
			songArtist = songArtistElements.size() == 0 ? "없음"
					: songArtistElements.stream().map(v -> v.getAttribute("title")).map(v -> v.split(" - ")[0])
